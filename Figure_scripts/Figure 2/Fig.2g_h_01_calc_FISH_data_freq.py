...
for chrom in chroms:
    ...
    data = np.load(f'{mci_data_dir}/chrom_subgraph/{chrom}_D_O3_subgraph.npy', allow_pickle=True)
    data_freq = np.load(f'{mci_data_dir}/chrom_subgraph/{chrom}_D_O3_subgraph_freq.npy', allow_pickle=True)
    ...
    fish_MCIs = np.array(list(combinations(chrom_fish_node_list, 3)))

    df_chrom = pd.DataFrame(columns=['bin1', 'bin2', 'bin3', 'freq'])
    for fish_mci in tqdm(fish_MCIs):
        n1, n2, n3 = fish_mci[0], fish_mci[1], fish_mci[2]
        bin_1, bin_2, bin_3 = node2bin[n1], node2bin[n2], node2bin[n3]
        ...
        df_chrom.loc[len(df_chrom.index)] = [bin_1, bin_2, bin_3, freq]
