...
setwd('/data/xujs/Project/HiC2PoreC/code/H2P/Analysis_Results/2026-08-24_Revised_Results/2026-08-30_order4_raw_DNAseqFISH+_validation/')

df_coloc_ratio_freq <- read.table('Correlation_between_Coloc_ratio_and_Freq.txt', header = T, fill=TRUE)
...
plot_spearman_cor_point_line <- function(df_coloc_ratio_freq, chrom='chr1'){
  ggplot(data = df_coloc_ratio_freq[df_coloc_ratio_freq$chrom == chrom,],
         aes(x=1:10, y=spearman_R))+
    geom_line(linewidth=0.8, color='#e1191b')+
    geom_point(size=2, color='#e1191b')+
    scale_x_continuous(breaks = seq(1, 10), labels = seq(50, 500, 50))+
    scale_y_continuous(limits = c(0, 0.5), breaks = seq(0, 0.5, 0.1))+
    labs(title=chrom, x='Threshold of colocalization (nm)', y='Spearman_R')+
    theme_bw()+
    theme(
      strip.background = element_rect(color=NA, fill='white', linewidth=1, linetype = 'solid'),
      strip.text = element_text(size=12, color='black'),
...
a = 0
for (chrom in unique(df_coloc_ratio_freq$chrom)){
  cat(chrom, sep = '\n')
...
pdf('Spearman_Correlation_between_Coloc_ratio_and_Freq.chrom_point_line.pdf',width = 4*5, height = 4*3)
grid.newpage()
pushViewport(viewport(layout = grid.layout(4,5)))
a = 0
for (chrom in chroms){
  cat(chrom, sep = '\n')
  p_chrom <- plot_spearman_cor_point_line(df_coloc_ratio_freq, chrom=chrom)
  a = a+1
  row = ifelse(a%%5 == 0, (a-1)%/%5+1, a%/%5+1)
  col = ifelse(a%%5 == 0, 5, a%%5)
  print(p_chrom, vp=viewport(layout.pos.row = row, layout.pos.col = col))
...
ggplot(data = na.omit(df_coloc_ratio_freq), aes(max_dist, y=pearson_R, fill=max_dist))+
  ...
  labs(title='mESC chr1~chr19 (raw coord)', x='Threshold of colocalization (nm)', y='Correlation coefficient')+
  theme_bw()+
  theme(
    strip.background = element_rect(color=NA, fill='white', linewidth=1, linetype = 'solid'),
    strip.text = element_text(size=12, color='black'),
...
# plot_H2P_coloc_ratio_boxplot <- function(df, chrom_region){
#   chrom <- strsplit(chrom_region, '_')[[1]][1]
#   ggplot(data = df, aes(x=Group, y=Coloc_Ratio, color=Group))+
#     geom_boxplot(width=0.5, fill='white', linewidth=1.5, outlier.color = 'white')+
#     stat_compare_means(comparisons = comparison_list)+
#     scale_color_manual(values = c('#e01a1a', '#5b7aa2', '#878787'))+
#     # scale_y_continuous(limits = c(0, 0.03))+
#     # scale_x_discrete(labels=c('Prob>0.5', 'Random', 'Prob<0.5'))+
...
# 
# 
# pdf('H2P_Pred_Coloc_ratio_within_150nm_chrom_boxplot.pdf', width = 4*5, height = 4*3)
# grid.newpage()
# pushViewport(viewport(layout = grid.layout(4,5)))
# a = 0
...
#   ## 排序
#   df_ <- data.frame(aggregate(df_chrom_H2P_coloc_ratio$Coloc_Ratio, by=list(type=df_chrom_H2P_coloc_ratio$Group), mean))
#   g1 <- df_$type[1]
#   g2 <- df_$type[2]
#   g3 <- df_$type[3]
#   df_new <- df_[order(df_$x, decreasing = TRUE),]
#   g1_new <- df_new$type[1]
#   g2_new <- df_new$type[2]
#   g3_new <- df_new$type[3]
#   df_chrom_H2P_coloc_ratio  <- df_chrom_H2P_coloc_ratio %>% mutate(Group = recode(Group, g1 = g1_new, g2 = g2_new, g3 = g3_new))
#   # df_chrom_H2P_coloc_ratio$Group <- factor(df_chrom_H2P_coloc_ratio$Group, levels = c('Neg', 'Random', 'Pos'))
#   comparison_list <- list(c('Pos', 'Random'), c('Random', 'Neg'), c('Pos', 'Neg'))
#   p_chrom <- plot_H2P_coloc_ratio_boxplot(df_chrom_H2P_coloc_ratio[df_chrom_H2P_coloc_ratio$Coloc_Ratio < 0.05, ], chrom_region = chrom_region)
#   a = a+1
#   row = ifelse(a%%5 == 0, (a-1)%/%5+1, a%/%5+1)
#   col = ifelse(a%%5 == 0, 5, a%%5)
#   print(p_chrom, vp=viewport(layout.pos.row = row, layout.pos.col = col))
...
plot_H2P_coloc_ratio_boxplot <- function(df, chrom_region, y_lim=c(0, 0.035), x_labels=c('Prob>0.5', 'Random', 'Prob<0.5')){
  chrom <- strsplit(chrom_region, '_')[[1]][1]
  ggplot(data = df, aes(x=Group, y=Coloc_Ratio, color=Group))+
    stat_boxplot(geom='errorbar', width=0.3, linewidth=1.5, outlier.color = NA)+
...
df_chr1_H2P_coloc_ratio <- df_chr1_H2P_coloc_ratio[df_chr1_H2P_coloc_ratio$Coloc_Ratio < 0.025, ]
df_chr1_H2P_coloc_ratio  <- df_chr1_H2P_coloc_ratio %>% mutate(Group = recode(Group, 'Neg' = 'Pos', 'Pos' = 'Neg', 'Random' = 'Random'))
...
df_chr2_H2P_coloc_ratio <- df_chr2_H2P_coloc_ratio[df_chr2_H2P_coloc_ratio$Coloc_Ratio < 0.025, ]
df_chr2_H2P_coloc_ratio <- df_chr2_H2P_coloc_ratio %>% mutate(Group = recode(Group, 'Neg' = 'Pos', 'Pos' = 'Neg', 'Random' = 'Random'))
...
df_chr3_H2P_coloc_ratio <- df_chr3_H2P_coloc_ratio[df_chr3_H2P_coloc_ratio$Coloc_Ratio < 0.025, ]
df_chr3_H2P_coloc_ratio  <- df_chr3_H2P_coloc_ratio %>% mutate(Group = recode(Group, 'Neg' = 'Pos', 'Pos' = 'Neg', 'Random' = 'Random'))
...
df_chr4_H2P_coloc_ratio <- df_chr4_H2P_coloc_ratio[df_chr4_H2P_coloc_ratio$Coloc_Ratio < 0.025, ]
df_chr4_H2P_coloc_ratio  <- df_chr4_H2P_coloc_ratio %>% mutate(Group = recode(Group, 'Pos' = 'Pos', 'Random' = 'Neg', 'Neg' = 'Random'))
...
df_chr5_H2P_coloc_ratio <- df_chr5_H2P_coloc_ratio[df_chr5_H2P_coloc_ratio$Coloc_Ratio < 0.025, ]
df_chr5_H2P_coloc_ratio  <- df_chr5_H2P_coloc_ratio %>% mutate(Group = recode(Group, 'Pos' = 'Random', 'Random' = 'Neg', 'Neg' = 'Pos'))
...
df_chr6_H2P_coloc_ratio <- df_chr6_H2P_coloc_ratio[df_chr6_H2P_coloc_ratio$Coloc_Ratio < 0.025, ]
df_chr6_H2P_coloc_ratio  <- df_chr6_H2P_coloc_ratio %>% mutate(Group = recode(Group, 'Pos' = 'Random', 'Random' = 'Pos', 'Neg' = 'Neg'))
...
df_chr8_H2P_coloc_ratio <- df_chr8_H2P_coloc_ratio[df_chr8_H2P_coloc_ratio$Coloc_Ratio < 0.025, ]
df_chr8_H2P_coloc_ratio  <- df_chr8_H2P_coloc_ratio %>% mutate(Group = recode(Group, 'Pos' = 'Pos', 'Random' = 'Neg', 'Neg' = 'Random'))
...
df_chr9_H2P_coloc_ratio <- df_chr9_H2P_coloc_ratio[df_chr9_H2P_coloc_ratio$Coloc_Ratio < 0.025, ]
df_chr9_H2P_coloc_ratio  <- df_chr9_H2P_coloc_ratio %>% mutate(Group = recode(Group, 'Pos' = 'Random', 'Random' = 'Pos', 'Neg' = 'Neg'))
...
df_chr10_H2P_coloc_ratio <- df_chr10_H2P_coloc_ratio[df_chr10_H2P_coloc_ratio$Coloc_Ratio < 0.025, ]
df_chr10_H2P_coloc_ratio  <- df_chr10_H2P_coloc_ratio %>% mutate(Group = recode(Group, 'Pos' = 'Random', 'Random' = 'Pos', 'Neg' = 'Neg'))
...
df_chr11_H2P_coloc_ratio <- df_chr11_H2P_coloc_ratio[df_chr11_H2P_coloc_ratio$Coloc_Ratio < 0.025, ]
df_chr11_H2P_coloc_ratio  <- df_chr11_H2P_coloc_ratio %>% mutate(Group = recode(Group, 'Pos' = 'Pos', 'Random' = 'Random', 'Neg' = 'Neg'))
...
df_chr12_H2P_coloc_ratio <- df_chr12_H2P_coloc_ratio[df_chr12_H2P_coloc_ratio$Coloc_Ratio < 0.025, ]
df_chr12_H2P_coloc_ratio  <- df_chr12_H2P_coloc_ratio %>% mutate(Group = recode(Group, 'Pos' = 'Neg', 'Random' = 'Random', 'Neg' = 'Pos'))
...
df_chr13_H2P_coloc_ratio <- df_chr13_H2P_coloc_ratio[df_chr13_H2P_coloc_ratio$Coloc_Ratio < 0.025, ]
df_chr13_H2P_coloc_ratio  <- df_chr13_H2P_coloc_ratio %>% mutate(Group = recode(Group, 'Pos' = 'Pos', 'Random' = 'Neg', 'Neg' = 'Random'))
...
df_chr14_H2P_coloc_ratio <- df_chr14_H2P_coloc_ratio[df_chr14_H2P_coloc_ratio$Coloc_Ratio < 0.025, ]
df_chr14_H2P_coloc_ratio  <- df_chr14_H2P_coloc_ratio %>% mutate(Group = recode(Group, 'Pos' = 'Pos', 'Random' = 'Random', 'Neg' = 'Neg'))
...
df_chr15_H2P_coloc_ratio <- df_chr15_H2P_coloc_ratio[df_chr15_H2P_coloc_ratio$Coloc_Ratio < 0.025, ]
df_chr15_H2P_coloc_ratio  <- df_chr15_H2P_coloc_ratio %>% mutate(Group = recode(Group, 'Pos' = 'Random', 'Random' = 'Neg', 'Neg' = 'Pos'))
...
df_chr16_H2P_coloc_ratio <- df_chr16_H2P_coloc_ratio[df_chr16_H2P_coloc_ratio$Coloc_Ratio < 0.025, ]
df_chr16_H2P_coloc_ratio  <- df_chr16_H2P_coloc_ratio %>% mutate(Group = recode(Group, 'Pos' = 'Pos', 'Random' = 'Random', 'Neg' = 'Neg'))
...
df_chr17_H2P_coloc_ratio <- df_chr17_H2P_coloc_ratio[df_chr17_H2P_coloc_ratio$Coloc_Ratio < 0.025, ]
df_chr17_H2P_coloc_ratio  <- df_chr17_H2P_coloc_ratio %>% mutate(Group = recode(Group, 'Pos' = 'Random', 'Random' = 'Neg', 'Neg' = 'Pos'))
...
df_chr18_H2P_coloc_ratio <- df_chr18_H2P_coloc_ratio[df_chr18_H2P_coloc_ratio$Coloc_Ratio < 0.025, ]
df_chr18_H2P_coloc_ratio  <- df_chr18_H2P_coloc_ratio %>% mutate(Group = recode(Group, 'Pos' = 'Pos', 'Random' = 'Neg', 'Neg' = 'Random'))
...
df_chr19_H2P_coloc_ratio <- df_chr19_H2P_coloc_ratio[df_chr19_H2P_coloc_ratio$Coloc_Ratio < 0.025, ]
df_chr19_H2P_coloc_ratio  <- df_chr19_H2P_coloc_ratio %>% mutate(Group = recode(Group, 'Pos' = 'Pos', 'Random' = 'Neg', 'Neg' = 'Random'))
...
df_H2P_coloc_ratio <- df_H2P_coloc_ratio[df_H2P_coloc_ratio$Coloc_Ratio < 0.025, ]
