...
# 2. 计算不同的maxdist四点在`所有细胞中共定位的比例`与`MCI freq`的`相关系数`：100/150/200/250/300/350/400/450/500nm
...
# data = np.load(f'{mci_data_dir}/all_3_Decompose_subgraph.npy')
# data_freq = np.load(f'{mci_data_dir}/all_3_Decompose_subgraph_freq.npy')
...
for chrom in chroms:
    ...
    data = np.load(f'{mci_data_dir}/chrom_subgraph/{chrom}_D_O4_subgraph.npy', allow_pickle=True)
    data_freq = np.load(f'{mci_data_dir}/chrom_subgraph/{chrom}_D_O4_subgraph_freq.npy', allow_pickle=True)
    mci_freq_dict = {tuple(data[i]):data_freq[i] for i in trange(len(data)) }

    chrom_fish_bin_list = df_fish_anno.loc[df_fish_anno.chrom == chrom, 'bin'].tolist()
    chrom_fish_node_list = [bin2node[bin_] for bin_ in chrom_fish_bin_list ]
    fish_MCIs = np.array(list(combinations(chrom_fish_node_list, 4)))

    df_chrom = pd.DataFrame(columns=['bin1', 'bin2', 'bin3', 'bin4', 'freq'])
    for fish_mci in tqdm(fish_MCIs):
        n1, n2, n3, n4 = fish_mci[0], fish_mci[1], fish_mci[2], fish_mci[3]
        bin_1, bin_2, bin_3, bin_4 = node2bin[n1], node2bin[n2], node2bin[n3], node2bin[n4]
        # index = np.where((data == (n1, n2, n3)).all(axis=1))
        # freq = [data_freq[index][0] if data_freq[index].shape[0] else 0][0]

        freq = mci_freq_dict.get(tuple(fish_mci), 0)
        df_chrom.loc[len(df_chrom.index)] = [bin_1, bin_2, bin_3, bin_4, freq]

    df_chrom.to_csv(f'{out_dir}/mESC_25Kb_FISH_MCI_Freq_{chrom}.csv', header=True, sep='\t', index=False)