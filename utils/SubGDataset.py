...
class GDataset:
    ...
    '''
    def __init__(self, x, edge_index, edge_attr, pos, y, subG_weight=None):
        self.x=x
...
class GDataloader(DataLoader):
    ...
    def get_subG_weight(self):
        return self.Gdataset.subG_weight
    ...
    def __next__(self):
        perm = next(self.iter)
        if self.Gdataset.subG_weight:
            return self.get_x(), self.get_ei(), self.get_ea(), \
                    self.get_pos()[perm], self.get_y()[perm], self.get_subG_weight()[perm]
        else:
...
class ZGDataloader(GDataloader):
    ...
    def __next__(self):
        ...
        return self.get_x(), self.get_ei(), self.get_ea(), \
                tpos, self.z_fn(self.get_x(), tpos), self.get_y()[perm], self.get_subG_weight()[perm]
